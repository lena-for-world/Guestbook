...
import projectB.guestbook.controller.PostForm;
...
public class WritingService {
    ...
    public void save(PostForm form) {
        Post post = Post.makeNewPost(form.getName(), form.getContent(), DateTime.now().toString("yyyy년MM월dd일 HH시mm분ss초"));
        if (validateContinuousPost(post)) {
            writingRepository.save(post);
        } else {
            throw new DateTimeException("방명록은 1분에 1개만 작성할 수 있습니다");
        }
        /* 동일한 이름 검증은 필요하지 않음! 게시글 구별을 이름으로 할 예정이기 때문 (∵ 로그인 구현이 없음)*/
    }
